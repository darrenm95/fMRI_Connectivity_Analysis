...
function nets_netweb(netF,netP,DD,sumpics,netwebdir,names)
...
system(sprintf('mkdir -p %s; (cd %s/netjs; tar cf - *) | (cd %s; tar xf -)',netwebdir,fCA_path,netwebdir));
...
fid = fopen(sprintf('%s/names.txt', NP), 'a');
